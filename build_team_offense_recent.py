...
faced, over four windows: season, last 30 days, last 14 days, last 7 days. Keyed by team id:

    "147": {
      "vsL": {"season": 104, "l30": 110, "l14": 96, "l7": 92},
      "vsR": {"season": 99,  "l30": 103, "l14": 101, "l7": 105},
      "paL30": 690, "paL14": 300, "paL7": 150
    }
  ...
  paL30 / paL14 / paL7 = plate appearances behind each recent window, so the app can regress thin
...
read that reliably anyway. The L7 window reacts fastest to a lineup change (e.g. a bat sitting) but
is the thinnest sample, so the app weights it lightly and falls back to L14 when it's empty.
...
def recent_windows(tid):
    """One game-log fetch -> ((l30_ops, pa30), (l14_ops, pa14), (l7_ops, pa7))."""
    ...
    cut7  = (date.today() - timedelta(days=WIN_L7)).isoformat()
    t30, t14, t7 = _new_tot(), _new_tot(), _new_tot()
    for s in splits:
        d = s.get("date", "")
        if not d or d < cut30:
            continue
        ...
        if d >= cut7:
            _add(t7, st)
    return ops_from_totals(t30), ops_from_totals(t14), ops_from_totals(t7)
...
def build_team(tid):
    ...
    if not ov or ov <= 0:
        return None
    (l30_ops, pa30), (l14_ops, pa14), (l7_ops, pa7) = recent_windows(tid)
    ...
    def side(season_hand):
        ...
        out["l7"]  = wrc(l7_ops  * ratio) if l7_ops  else out["l14"]
        return out

    return {"vsL": side(vL), "vsR": side(vR),
            "paL30": round(pa30), "paL14": round(pa14), "paL7": round(pa7)}
...
def main():
    ...
    print("wrote %s : %d teams (windows: season / L%d / L%d / L%d)"
          % (OUT_PATH, len(out), WIN_L30, WIN_L14, WIN_L7), file=sys.stderr)
